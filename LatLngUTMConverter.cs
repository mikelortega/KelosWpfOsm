...
public class LatLngUTMConverter
{
    ...
    public int UtmZoneNumber(double latitude, double longitude)
    {
        int ZoneNumber;
    ...
    private string UtmLetterDesignator(double latitude)
    {
    ...
    public UTMResult LatLngToUtm(double latitude, double longitude)
    {
        int zone = UtmZoneNumber(latitude, longitude);

        return LatLngToUtm(latitude, longitude, zone);
    }

    public UTMResult LatLngToUtm(double latitude, double longitude, int zone)
    {
        if (status)
        ...
        var LongOrigin = (zone - 1) * 6 - 180 + 3;  //+3 puts origin in middle of zone
        var LongOriginRad = LongOrigin * toRadians;

        var UTMZone = UtmLetterDesignator(latitude);
        ...
        return new UTMResult { Easting = UTMEasting, Northing = UTMNorthing, ZoneNumber = zone, ZoneLetter = UTMZone };
    }
    ...
    public double GridConvergence(double latitude, double longitude)
    {
        int zone = UtmZoneNumber(latitude, longitude);

        return GridConvergence(latitude, longitude, zone);
    }
